...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Betentrycntr extends CI_Controller {
		...
		function CheckAdminPass(){
			$condition=$this->Betentrymodel->CheckAdminPass();		
			if ($condition[0]['resultV']==1) {
		...
		function deleteGetbettingmat($betId,$userId,$marketId){
			if($this->session->userdata('user_id')==1 && $this->session->userdata('type')==0){
				...
				if ($condition) {
					...
					$data['error'] = 1;
					$data['message'] = "You Don't Have Permission to Delete This Record....";
					echo json_encode($data);	
			}
		...
		function chkbets(){//170131
				$data['jsonData']=$this->Betentrymodel->chkbets();
				$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}
		...
		function UpdateNotification(){
			$condition=$this->Betentrymodel->UpdateNotification();				
				if ($condition== true) {
					$data['error'] = 0;
		...
		function Getnotification(){
			$data['GetNotification']=$this->Betentrymodel->GetNotification();
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}
		function getNotifi_status($userId){
			$data['jsonData']=$this->Betentrymodel->getNotifi_status($userId);
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}
		function updatenoti_status($userId){
			$data['jsonData']=$this->Betentrymodel->updatenoti_status($userId);
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}
