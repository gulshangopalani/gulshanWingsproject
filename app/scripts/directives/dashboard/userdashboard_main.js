...
angular.module('ApsilonApp').controller('userdashboard', function ($scope, $http, sessionService, $timeout, deviceDetector, $filter) {
    ...
    /*end of slider*/
    $scope.datacolumns = [{"id": "TeamA", "type": "bar", "name": "Team A"}, {
        "id": "TeamB",
        "type": "bar",
        "name": "Team B"
    }, {"id": "theDraw", "type": "bar", "name": "Draw"}];
    $scope.datapoints = [{}];
                    ...
                    if ($scope.oddsDetail != angular.isUndefinedOrNull)
                        $scope.oddsDetail.find(function (item, itemIndex) {
                            ...
                            if (selectedRunner != angular.isUndefinedOrNull && selectedRunner.length > 0 && selectedRunner[0].status == "OPEN" && selectedRunner[0].totalMatched > $scope.oddsDetail[itemIndex].totalMatched) {
                                try {
                                    $scope.oddsDetail[itemIndex].runners[0].ex.availableToBack[0].price = selectedRunner[0].runners[0].ex.availableToBack[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[0].ex.availableToBack[0].price = ""
                                    } catch (e) {
                                    }
                                }
                                try {
                                    $scope.oddsDetail[itemIndex].runners[1].ex.availableToBack[0].price = selectedRunner[0].runners[1].ex.availableToBack[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[1].ex.availableToBack[0].price = ""
                                    } catch (e) {
                                    }
                                }
                                try {
                                    $scope.oddsDetail[itemIndex].runners[2].ex.availableToBack[0].price = selectedRunner[0].runners[2].ex.availableToBack[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[2].ex.availableToBack[0].price = ""
                                    } catch (e) {
                                    }
                                }
                                try {
                                    $scope.oddsDetail[itemIndex].runners[0].ex.availableToLay[0].price = selectedRunner[0].runners[0].ex.availableToLay[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[0].ex.availableToLay[0].price = ""
                                    } catch (e) {
                                    }
                                }
                                try {
                                    $scope.oddsDetail[itemIndex].runners[1].ex.availableToLay[0].price = selectedRunner[0].runners[1].ex.availableToLay[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[1].ex.availableToLay[0].price = ""
                                    } catch (e) {
                                    }
                                }
                                try {
                                    $scope.oddsDetail[itemIndex].runners[2].ex.availableToLay[0].price = selectedRunner[0].runners[2].ex.availableToLay[0].price;
                                } catch (e) {
                                    try {
                                        $scope.oddsDetail.runners[2].ex.availableToLay[0].price = ""
                                    } catch (e) {
                                    }
                                }
                            }
    ...
    $scope.getUrl = function (type, matchid, marketid, matchname, matchdate, SportId)//sourabh 161231
    {
        switch (type) {
            case "0":
                return "Matchodds({MatchId: " + matchid + ",MarketId:" + marketid + ",matchName:'" + matchname.replace("'", "&quot;") + "',date:'" + matchdate + "'})";
                break;
            case "1":
                return "Evenoddfancy({matchId: " + matchid + ",FancyID:" + marketid + ",TypeID:" + type + ",matchName:'" + matchname.replace("'", "&quot;") + "',sportId:4 })";
                break;
            case "2":
                return "Sessionfancy({matchId: " + matchid + ",FancyID:" + marketid + ",TypeID:" + type + ",sportId:" + SportId + ",matchName:'" + matchname.replace("'", "&quot;") + "'})";
                break;
            case "3":
                return "Khaddalfancy({matchId: " + matchid + ",FancyID:" + marketid + ",TypeID:" + type + ",matchName:'" + matchname.replace("'", "&quot;") + "',sportId:4 })";
                break;
            case "4":
                return "Lastdigit({matchId: " + matchid + ",FancyID:" + marketid + ",TypeID:" + type + ",matchName:'" + matchname.replace("'", "&quot;") + "',sportId:4 })";
                break;
            case "5":
                return "Updown({matchId: " + matchid + ",FancyID:" + marketid + ",TypeID:" + type + ",matchName:'" + matchname.replace("'", "&quot;") + "',sportId:4 })";
                break;
        }
    }
