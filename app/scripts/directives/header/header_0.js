    ...
    return {
        ...
        controller: ['$scope', '$http', 'loginService', '$mdDialog', '$window', 'Dialog', 'sessionService','$interval','get_userser','$state','speech',function ($scope, $http, loginService, $mdDialog, $window, Dialog, sessionService,$interval,get_userser,$state,speech) {
           ...
           /* var chkFancyDisplay=function check_Fancydisplay() {
            ...
            $scope.getFancyStatus = function (fancyId, active, autoActive) {
                var formData = { id: fancyId, active: active, HelperID: sessionService.get('HelperID'), autoActive: autoActive}
                $http({
                    method: 'POST',
