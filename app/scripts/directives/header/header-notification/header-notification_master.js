...
angular.module('ApsilonApp').directive('headerNotificationmaster', ['$timeout', function ($timeout) {
    return {
        ...
        controller: function ($scope, $http, loginService, $mdDialog, sessionService, $rootScope, get_userser, Dialog,$state) {
            ...
            var chkBalStatus = function () { get_userser.updateUserBal(sessionStorage.slctUseID, function (response) {	/*console.log("service Called"+response);*/ }); }
            setInterval(chkBalStatus, 5000000);
            $http.get('Chipscntrl/getChipDataById/' + sessionStorage.user_id).success(function (data, status, headers, config) {
                $scope.cipsData = data.betLibility;
