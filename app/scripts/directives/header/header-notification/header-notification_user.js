    ...
    return {
        ...
        link: function (scope, element, attrs,Matchoddscntr) {            
           // Matchoddscntr.test_dir();
            scope.$on('updateBal', function (event, data) { scope.updateBaluserBal(); });
        ...
        controller: function ($scope, $http, loginService, $mdDialog, sessionService, $rootScope, get_userser, Dialog,$state) {
            ...
            var chkBalStatus = function () { get_userser.updateUserBal(sessionStorage.slctUseID, function (response) {	
            /*console.log("service Called"+response);*/
            if(sessionStorage.user_id==angular.isUndefinedOrNull){
                 $scope.logout();
            }

            }); }
           //setInterval(chkBalStatus, 500000);
           $scope.updateBaluserBal=function(){
            $http.get('Chipscntrl/getChipDataById/' + sessionStorage.user_id).success(function (data, status, headers, config) {
                $scope.cipsData = data.betLibility;
